...
async function assertOwnership(id: string, tenantId: string) {
  const existing = await pool.query('SELECT tenant_id FROM project_announcements WHERE id = $1', [id])
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await pool.query(
      `UPDATE project_announcements SET ${updates.join(', ')} WHERE id = $${i} RETURNING *`,
      values
    )
