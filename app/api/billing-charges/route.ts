import { NextRequest, NextResponse } from 'next/server'
import pool from '@/lib/db'
import { getCurrentUser } from '@/lib/auth'
import { logActivity } from '@/lib/activityLog'

export async function POST(req: NextRequest) {
  try {
    const user = await getCurrentUser()
    if (!user) return NextResponse.json({ error: 'Not authenticated' }, { status: 401 })
    if (user.role !== 'admin') return NextResponse.json({ error: 'Not authorized' }, { status: 403 })

    const body = await req.json()
    const { project_id, description, hours, rate, amount, charge_date } = body

    if (!project_id || !description) {
      return NextResponse.json({ error: 'project_id and description are required' }, { status: 400 })
    }

    const proj = await pool.query('SELECT tenant_id FROM projects WHERE id = $1', [project_id])
    if (proj.rows.length === 0 || proj.rows[0].tenant_id !== user.tenantId) {
      return NextResponse.json({ error: 'Project not found' }, { status: 404 })
    }

    const computedAmount =
      amount !== undefined && amount !== null && amount !== ''
        ? amount
        : (hours && rate ? Number(hours) * Number(rate) : null)

    const result = await pool.query(
      `INSERT INTO billing_charges (tenant_id, project_id, description, hours, rate, amount, charge_date, source)
       VALUES ($1, $2, $3, $4, $5, $6, $7, 'manual')
       RETURNING *`,
      [user.tenantId, project_id, description, hours || null, rate || null, computedAmount, charge_date || null]
    )

    await logActivity({
      tenantId: user.tenantId,
      projectId: result.rows[0].project_id,
      module: 'project_center',
      entityType: 'billing_charge',
      entityId: result.rows[0].id,
      action: 'created',
      actorName: user.name,
      actorEmail: user.email,
      summary: result.rows[0].description,
      detail: result.rows[0].amount != null ? `Amount: $${result.rows[0].amount}` : null,
    })

    return NextResponse.json({ success: true, charge: result.rows[0] })
  } catch (error) {
    return NextResponse.json({ error: String(error) }, { status: 500 })
  }
}
