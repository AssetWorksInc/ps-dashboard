...
async function assertOwnership(id: string, tenantId: string) {
  const existing = await pool.query('SELECT tenant_id FROM meeting_notes WHERE id = $1', [id])
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (check === 'not_found') return NextResponse.json({ error: 'Note not found' }, { status: 404 })
    ...
    const result = await pool.query(
      `UPDATE meeting_notes SET ${updates.join(', ')} WHERE id = $${i} RETURNING *`,
      values
    )
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const deleted = await pool.query('DELETE FROM meeting_notes WHERE id = $1 RETURNING *', [id])
    ...
    await logActivity({
      ...
      actorName: user.name,
      actorEmail: user.email,
      summary: deleted.rows[0]?.title || 'Meeting note',
    })
