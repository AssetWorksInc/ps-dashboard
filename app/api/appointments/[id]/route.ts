...
const EDITABLE_FIELDS = ['title', 'session_type', 'consultant', 'scheduled_at', 'location', 'notes']

async function assertOwnership(id: string, tenantId: string) {
  const existing = await pool.query('SELECT tenant_id FROM appointments WHERE id = $1', [id])
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (check === 'not_found') return NextResponse.json({ error: 'Appointment not found' }, { status: 404 })
    ...
    const result = await pool.query(
      `UPDATE appointments SET ${updates.join(', ')} WHERE id = $${i} RETURNING *`,
      values
    )
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const deleted = await pool.query('DELETE FROM appointments WHERE id = $1 RETURNING *', [id])
    ...
    await logActivity({
      ...
      actorName: user.name,
      actorEmail: user.email,
      summary: deleted.rows[0]?.title || 'Appointment',
    })
