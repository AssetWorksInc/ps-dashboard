...
const EDITABLE_FIELDS = ['name', 'role', 'department', 'email', 'phone', 'bio', 'is_ps_team']

async function assertOwnership(id: string, tenantId: string) {
  const existing = await pool.query('SELECT tenant_id FROM team_directory WHERE id = $1', [id])
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (check === 'not_found') return NextResponse.json({ error: 'Team member not found' }, { status: 404 })
    ...
    const result = await pool.query(
      `UPDATE team_directory SET ${updates.join(', ')} WHERE id = $${i} RETURNING *`,
      values
    )
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await pool.query('DELETE FROM team_directory WHERE id = $1', [id])
