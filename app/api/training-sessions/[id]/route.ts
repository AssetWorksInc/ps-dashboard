...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const deleted = await pool.query(
      `DELETE FROM training_sessions WHERE id = $1 RETURNING id, title, presenter`,
      [id]
    )
    ...
    await logActivity({
      ...
      actorName: user.name,
      actorEmail: user.email,
      summary: deleted.rows[0]?.title || 'Training session',
      detail: deleted.rows[0]?.presenter || null,
    })
